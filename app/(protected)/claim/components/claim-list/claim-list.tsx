...
type IClaimListGridContentItems = Array<IClaimListGridContentItem>;

type SearchResultsType = 'card' | 'list';

export function ClaimList({ mode }: { mode: SearchResultsType }) {
  ...
  const items: IClaimListGridContentItems = [
    {
      logo: '11.png',
  ...
  const renderItem = (item: IClaimListGridContentItem, index: number) => {
    ...
    return <ClaimDetail key={index} {...props} />;
